...
public class implementHashMapsInJava {
    ...
    public static class HashMap<K, V> {
        ...
        private void rehash() throws Exception {
          ...
          size = 0;   // since we call put function, size would be increased everytime on insertion
    
          for (int bi = 0; bi < oba.length; bi++) {
            for (HMNode onode : oba[bi]) {
              put(onode.key, onode.value); // this will put the element from original bucket to the renewed bucket
            }
          }
        }
