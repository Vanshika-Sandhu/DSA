...
public class iterativePreAndPostOrderTraversal {
    private static class Node {
      ...
      public static void IterativePreandPostOrder(Node node) {
        ...
        while(st.size()>0){
            Pair top = st.peek();
            if(top.state == -1){
            // insert in preorder, here state = -1
                top.state++;
                pre += top.node.data+ " ";
            }else if(top.state == top.node.children.size()){
                // insert in post order, here state = children.size()
                post += top.node.data+ " ";
                st.pop();
            }else{
                // insert the child at index = state, here state = 0 to children.size-1
