...
public class removeLeavesGenTree {
    ...
    private static class Node {
      ...
      public static void removeLeaves(Node node) {
        //   this part is written in preorder so that all the nodes do not get deleted
        // loop in reverse so that no element is left/skipped
        for(int i=node.children.size()-1; i>=0; i--){
            Node child = node.children.get(i);
        ...
        //   faith => child node removes all leaf nodes in their respective tree
        for(Node child: node.children){
            removeLeaves(child);
        }
      }
