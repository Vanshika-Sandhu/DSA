...
public class predecessorAndSuccessorOfElm {
    private static class Node {
      ...
      public static void predecessorAndSuccessor(Node node, int data) {
        ...
        if(state == 0){
            if(node.data == data){
            // if element is found state is changed
                state = 1;
            }else{
                // else the predecessor is updaed till it reaches one node before the element
                predecessor = node;
            }
        }else if(state == 1){
            // successor is reached when state = 1
            successor = node;
            state = 2;
        }
